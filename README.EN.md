...
- **Village Structure Injection**: `VillageStructureInjector` replaces `town_centers` template pools in 5 vanilla village types (plains/desert/savanna/snowy/taiga) with custom meeting point structures containing teleport anchors
...
### 12. Animation Subtitles System

An entity-based dynamic subtitle system with per-character pop-in animation and shatter-split effects, designed for NPC dialogue, cutscene narration, and more.

- **Implementation**: `AnimationSubtitleEntity` (non-pickable, non-pushable, 256-block render distance)
- **Dual-Mode Architecture**: Main subtitle (per-character pop-in → hold → per-character drop into shards) and shards (physics fall → settle → rest → shrink and vanish)
- **Command**: `/yzwc function animation_subtitles` (requires OP 4 or equivalent)
  - `set pos <pos> <rot1> <rot2> <text> [time]` — Spawn at specific coordinates with rotation
  - `set player_location <text> [time] [player]` — Spawn 2 blocks in front of target player, auto-match orientation
- **Format Code Support**: Uses `&` as § format code prefix, supports colors (0-9, a-f) and styles (k/l/m/n/o/r)
- **Animation Parameters**: Pop interval 4 ticks, pop duration 8 ticks, default hold 100 ticks (5s), drop interval 1 tick
- **Shard Physics**: Gravity 0.018, ground bounce 0.22, settling 6 ticks, resting 90 ticks, shrinking 20 ticks

### 13. Adventure XP System
...
Fine-grained LuckPerms-based permission control with automatic OP-level fallback. **20 distinct permission nodes** including `account.mgr.*`, `command.*`, and `*` wildcards.

### 16. Creative Mode Tabs
...
│   ├── Permission: youzaiworldcore.command.function.animation_subtitles (OP 4)
│   ├── set pos <pos> <rot1> <rot2> <text> [time]  → Spawn animated subtitle at coords
│   └── set player_location <text> [time] [player]  → Spawn in front of target player
│
...
| `youzaiworldcore.command.function.animation_subtitles` | Animation subtitles | OP 4 |
...
│   ├── entity/animation_subtitle/        # Animation subtitle entity system
...
│   ├── worldgen/                         # World generation (village structure injector)
...
│   ├── mixin/client/                     # Client Mixins (title, options, button, pause, chat, loading, seat, rendering, etc.; 19 total)
