    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        var enableNode = Commands.literal("enable")
                ...
                .then(Commands.argument("enabled", BoolArgumentType.bool())
                        .requires(src -> LuckPermsHelper.checkPermission(
                                src, LuckPermsHelper.PERMISSION_EVENT_SET, Commands.LEVEL_ADMINS))
                        .executes(ctx -> setEnable(ctx, BoolArgumentType.getBool(ctx, "enabled")))
                );
        ...
        var chanceNode = Commands.literal("chance")
                ...
                .then(Commands.argument("chance", DoubleArgumentType.doubleArg())
                        .requires(src -> LuckPermsHelper.checkPermission(
                                src, LuckPermsHelper.PERMISSION_EVENT_SET, Commands.LEVEL_ADMINS))
                        .executes(ctx -> setChance(ctx, DoubleArgumentType.getDouble(ctx, "chance")))
                );
