...
import top.csituka.youzaiworldcore.enchlevellangpatch.api.EnchantmentLevelLangPatch;
import top.csituka.youzaiworldcore.enchlevellangpatch.api.EnchantmentLevelLangPatchConfig;
...
import static top.csituka.youzaiworldcore.enchlevellangpatch.impl.NumberFormatUtil.intToRomanImpl;
...
public final class LangPatchImpl {
    ...
    public static void init() {
        LOGGER.info(MARKER, "Initializing Enchantment Level Language Patch integration");
        ...
        LOGGER.info(MARKER, "Enchantment Level Language Patch initialized ({} patches registered)", PREDICATES.size());
    }

    private static void applyConf4() {
        Class<?> c;
        try {
            c = Class.forName("top.csituka.youzaiworldcore.enchlevellangpatch.ext.conf4.LangPatchConfigHooks");
    ...
    /**
     * Called from the ClientLanguage mixin. Injects into getOrDefault head.
     */
    public static @Nullable String hookWithFallback(String key, Map<String, String> translations, String fallback) {
        return loop((keyPredicate, valueMapping) -> {
            if (!keyPredicate.test(key)) return null;
