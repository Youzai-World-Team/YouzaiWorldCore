...
public final class CosmeticSnapshotHasher {
     ...
     */
    public static String hash(byte[] skinWide, byte[] skinSlim, byte[] cloakWide, byte[] cloakSlim) {
        try {
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            updateSlot(digest, skinWide);
            updateSlot(digest, skinSlim);
            updateSlot(digest, cloakWide);
            updateSlot(digest, cloakSlim);
