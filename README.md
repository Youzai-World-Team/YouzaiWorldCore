...
| **生存玩家** | 在服务器游玩的普通玩家，使用悠哉工具、成就系统、世界传送等 |
...
| 配置项 | 说明 |
|--------|------|
| **开发者模式** | 启用开发者模式（服务端专用，与日志输出结合控制调试日志） |
| **日志输出** | 将详细的噪音日志（实验性功能注册、配置加载、账户数据等）写入 latest.log |

开发者模式（`devModeEnabled`）和日志输出（`logToFile`）双开关协同工作：仅当两者同时启用时，`DebugLogger` 才会产生调试日志输出。

### 12. 调试日志系统（DebugLogger）

一套专业的调试日志工具类，受双开关协同控制，提供丰富的日志追踪方法：

| 方法类别 | 说明 |
|---------|------|
| **entering / exiting** | 方法进入/退出追踪，可选参数/返回值记录 |
| **branch** | 条件分支判断记录，含决策结果和上下文信息 |
| **stateChange** | 状态变更记录，支持旧值→新值对比 |
| **exception** | 异常捕获块记录，支持完整堆栈或摘要 |
| **trace / debug / info / warn / error** | 标准日志级别 |

日志格式：`[yyyy-MM-dd HH:mm:ss.SSS] [LEVEL] [Module] 描述信息`

### 13. 窗口定制化

- **自定义窗口图标**：使用模组资源中的 `jar_icon.png` 替换 Minecraft 默认窗口图标（任务栏和标题栏）
- **自定义窗口标题**：通过 `WindowTitleMixin` 拦截 `Window.setTitle()` 调用，将窗口标题替换为模组自定义标题（格式：`YouzaiWorldCore 1.12.5+indev — Minecraft 26.2`）

### 14. 标题界面改造

通过 `TitleScreenMixin` 对 Minecraft 主菜单界面进行了全面改造：

- **自定义按钮**：加入服务器、选项、退出游戏按钮，使用自定义 `TitleScreenTextButton` 控件
- **公告横幅**：在标题界面上方展示模组公告信息（包含标题、服务器简介文字），带动画淡入效果
- **渐变背景**：通过 `ScreenMixinForProgressBg` 拦截加载界面背景绘制，增加渐变背景效果
- **Logo 纹理**：通过 `LogoTextureMixin` 替换 Mojang 加载界面 Logo 纹理为自定义资源
- **调试模式**：开发者模式下额外显示"测试页"按钮

### 15. CI/CD 构建

项目配置了 GitHub Actions 持续集成工作流（`.github/workflows/build.yml`）：

- 自动在 **Ubuntu / Windows / macOS** 三平台构建
- JDK 25
- 构建成功后自动上传 Linux 构建产物为 Artifact

### 16. 维度池系统（Dimension Pool）
...
│   ├── placeholders/                           # 占位符系统
│   ├── screen/                                 # 容器菜单
│   └── util/                                   # 工具类（DebugLogger 等）
│
├── client/java/top/csituka/youzaiworldcore/    # 客户端专用代码
│   ├── client/Client.java                      # 客户端入口（含窗口图标设置）
│   ├── config/                                 # 客户端外部设置
│   ├── higherchat/                             # 聊天显示优化
