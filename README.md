...
`ScoreboardSidebarMixin` 在「使用 YZUI」或「显示 YZHUD」任一开启时接管原版侧边栏记分板，将其替换为半透明白色圆角面板，同时保留原版的目标选择、隐藏条目过滤、分数排序、队伍颜色/前后缀和数字格式；两个开关都关闭时完全保留原版记分板，其位置也固定为原版布局不可更改，YZHUD 自定义页面中的记分板随之置灰锁定、不可选中或拖拽。YZHUD 自定义页面可分别拖拽物品栏、装备栏、状态效果和记分板四个组件，并通过统一滑块实时调整透明度；位置以 `-1..1` 的归一化位移保存，记分板会按当帧实际宽高重新换算位置，内容变化后仍保持在屏幕范围内。

`InventoryHudMixin` 还会在左下角绘制物品栏、装备与状态效果面板。三个面板以 1920×1080、界面尺寸 3 对应的 640×360 GUI 坐标为设计基准，根据当前 GUI 宽高的较小缩放比统一缩放，使面板、槽位、物品、图标、文字与间距在不同分辨率、窗口大小和界面尺寸下保持一致比例。状态效果面板位于装备栏右侧、物品栏上方，仅在玩家拥有状态效果时显示；面板底边固定并随效果数量向上增长，按获得顺序自下而上列出效果图标、名称、罗马数字等级与剩余时间，最后 10 秒对应的完整行或简略单元格闪烁，并替代原版右上角效果图标。面板最多占用 13 行；超过 13 个效果后，从最底行开始依次压缩为 2、3、4 个简略单元格，填满 4 个后才继续压缩上一行；简略行内新效果在左、旧效果在右。简略单元格省略名称与等级，只在图标右下角叠加剩余时间；最多显示最新 52 个效果，保证新获得的效果优先可见。

装备栏面板底部依次显示箭矢总数（普通箭、药水箭与光灵箭）、烟花火箭总数（合并所有飞行时间与烟花效果组件）以及背包和快捷栏空位数。

三个面板均带有连续动画反馈：物品进入空槽时淡入闪烁，离开槽位时淡出缩小；物品或指示器数量增加时图标与文字同步弹性闪烁；装备和道具的剩余耐久不高于 10% 时逐渐增强摇晃。状态效果新获得时拉伸淡入，移除时收缩淡出，因新增、移除或行压缩产生的位置与宽度变化会平滑滑动。额外反馈包括数量增加的绿色高光、耐久修复的青色脉冲，以及低耐久时的红色呼吸警告。
...
- **小地图与全屏地图**：圆形、方形、圆角边框，旋转、缩放、拖动、区块网格、坐标与群系信息。小地图以 960×540 GUI 单位为设计基准等比缩放到当前界面尺寸，占屏比例不随「界面尺寸」设置变化（`size` 即该基准下的边长，设置页会同时显示实际显示尺寸）。
...
│   ├── hud/                              # 魔力条 / 冒险等级 HUD / YZUI 物品栏·装备·状态效果·记分板·生命·饥饿·盔甲·氧气条 / 热键栏 / 工具信息覆盖
