...
`ScoreboardSidebarMixin` 接管原版侧边栏记分板，将其替换为半透明白色圆角面板，同时保留原版的目标选择、隐藏条目过滤、分数排序、队伍颜色/前后缀和数字格式。YZHUD 自定义页面可分别拖拽物品栏、装备栏、状态效果和记分板四个组件，并通过统一滑块实时调整透明度；位置以 `-1..1` 的归一化位移保存，记分板会按当帧实际宽高重新换算位置，内容变化后仍保持在屏幕范围内。
